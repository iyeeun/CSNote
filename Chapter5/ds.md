...
- 힙 : 완전 이진 트리 기반의 자료 구조, minheap과 maxheap이 있음
    - 종류
        - 최대힙 : 루트의 키가 모든 자식의 키 중에서 가장 큼 (재귀적)
        - 최소힙 : 루트의 키가 모든 자식의 키 중에서 가장 작음 (재귀적)
    - 삽입<br>: 마지막 노드에 삽입 후 부모 노드와 크기를 비교하며 교환
    - 삭제<br> : 루트를 삭제하고, 마지막 노드와 루트를 스왑하여 계속 스왑하며 재구성됨

### 3.4 우선순위 큐 (Priority Queue)
- 우선순위 큐 : 큐에서 우선순위가 높은 요소가 먼저 제공 -> 힙 기반

### 3.5 맵 (Map)
- 맵 : 특정 순서에 따라 키와 매핑된 값의 조합으로 형성됨

### 3.6 셋 (Set)
- 셋 : 특정 순서에 따라 중복 없이 고유한 요소를 저장

### 3.7 해시 테이블 (Hash Table)
- 해시 테이블 : 무한에 가까운 데이터들을 유한한 개수의 해시 값으로 매핑
    - 삽입, 삭제, 탐색이 평균적으로 O(1)
    - unordered_map으로 구현
